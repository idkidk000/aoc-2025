...
function part2(points: Point2DLike[], rects: Rect[], logger: Logger) {
  ...
  // this would not handle touching parallel lines in `lines` which open out into a hole inside `rect`
  function isRectInside(rect: Rect): boolean {
    ...
    // brute force the compressed coordinate space
    const rectYs = ys.filter((y) => y > rect.minY && y < rect.maxY);
    for (const x of xs.filter((x) => x >= rect.minX && x <= rect.maxX))
      for (const y of rectYs) if (!isPointInside({ x, y })) return false;
    return true;
  }
